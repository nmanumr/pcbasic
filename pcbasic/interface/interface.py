...
class Interface(object):
    ...
    def quit_input(self):
        ...
        while not self._audio_queue.empty():
            try:
                if type(self._audio_queue) is asyncio.Queue:
...
class InterfaceAsync(Interface):
    def __init__(self, try_interfaces=(), audio_override=None, wait=False, **kwargs):
        """Initialise interface."""
        ...
        self._input_queue.put_wait = self._input_queue.put
        self._input_queue.put = self._input_queue.put_nowait

        self._video_queue.put_wait = self._video_queue.put
        self._video_queue.put = self._video_queue.put_nowait

        self._audio_queue.put_wait = self._audio_queue.put
        self._audio_queue.put = self._audio_queue.put_nowait
    ...
    def launch(self, target, *args, **kwargs):
        return asyncio.gather(
            self._session_runner(target, *args, **kwargs),
            self.run()
        )

    async def _session_runner(self, target, *args, **kwargs):
        """Session runner."""
        try:
